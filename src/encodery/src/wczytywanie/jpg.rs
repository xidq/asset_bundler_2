...
pub fn jpeg(bajty: &Vec<u8>) -> Result<DaneDoWczytywania, std::io::Error> {
    ...
    if let Ok(jpeg_struct) = Jpeg::from_bytes(bytes::Bytes::copy_from_slice(bajty)) {
        if let Some(exif_data) = jpeg_struct.exif() {
            exif_out = Some(exif_data.to_vec());
        }
        if let Some(icc_data) = jpeg_struct.icc_profile() {
            typ_koloru = ColorProfilePhoto::ICC(icc_data.to_vec());
        }
    }
