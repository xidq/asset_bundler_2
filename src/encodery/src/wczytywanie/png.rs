...
pub fn png(bajty: &Vec<u8>) -> Result<DaneDoWczytywania, std::io::Error> {
    ...
    if let Some(icc_chunk) = &reader.info().icc_profile {
        // PNG przechowuje profil ICC w formie skompresowanej lub surowej wewnątrz iCCP.
        // Biblioteka 'png' zazwyczaj zwraca już rozpakowane bajty.
        if !icc_chunk.is_empty() {
            typ_koloru = ColorProfilePhoto::ICC(icc_chunk.to_vec());
        }
    }
