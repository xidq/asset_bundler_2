...
use enumy::rozszerzenia::rozszenienia_zdjec::ImgExtFf;
...
pub fn ff<'a>(dane: &'a DaneKonw, kolor: &'a Color, jezyk: &'a WybórJęzyka, temat: &'a UstawieniaThemeWsio) -> Element<'a, Message> {
    Column::new()
        .push(przycisk_rozszerzenia(ImgExtTag::Ff, ButtonType::KonwRozszerzenia, kolor, if dane.rozszerzenia.ff.is_some() { &BtnState::Active } else { &BtnState::Disabled }, jezyk, temat))
        .push(
            container(
                Row::new()
                    .push(
