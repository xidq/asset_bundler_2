...
use zstd::*;
use crate::io::enums_structs_io::{KolejnośćDziałań, PoziomKompresji, ProcesStatus, Progress};

pub async fn kompresujsuj(
    ścieżka_pliku:PathBuf,
    nazwa_pliku:String,
    poziom_kompresji:PoziomKompresji,
    mut tx: mpsc::Sender<crate::io::enums_structs_io::Progress> // Dodajemy kanał tutaj
    ) ->  Result<(), tokio::io::Error>{

    let _ = tx.send(Progress::ProcesKompresji {
        etap: KolejnośćDziałań::Kompresja,
        procent: None,
    ...
    println!("DEBUG KOMPRESJA: Czy plik istnieje? {}", sciezka_temp.exists());
    ...
    let poziom = match poziom_kompresji {
        PoziomKompresji::Brak => 0,
        PoziomKompresji::Standard => 3,
        PoziomKompresji::Duża => 10,
    };
