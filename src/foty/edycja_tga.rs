...
use crate::foty::zmiana_fot::{Interpolacja, Rozdzielczości, Obraz, Rozszerzenia, zaszumianie, Zaszumianie, PostepMieleniaZdjec, ObrazTGA};
...
pub async fn edycja_tga(
    ...
    for wariant in rozdzielczości {
        ...
        //     let _ = tx.send(PostepMieleniaZdjec::Rozpoczęto(1,*procent_progress)).await;
        // }
        ...
        for fdgfshd in bit_depth {
            ...
            *oopr += 1;
            ...
            let (final_img, nazwa_bd,color_type) = match fdgfshd {
                ObrazTGA::Szary8 =>{
                        ...
                        let res = if docelowy_wymiar == 0 {
                            ...
                            match zaszumianie_zmienna{
                                Some(x) => zaszumianie(x,img.resize(docelowy_wymiar, docelowy_wymiar, filtr)),
                                None => img.resize(docelowy_wymiar, docelowy_wymiar, filtr),}.to_luma8()
                        // img.resize(docelowy_wymiar, docelowy_wymiar, filtr).to_luma8()
                        };
                        (res.into_raw(), "_g", image::ExtendedColorType::L8)
                    },
                ...
                ObrazTGA::HighColor16 => {
                    ...
                    let xxx = if docelowy_wymiar == 0 {
                        match zaszumianie_zmienna{
                            ...
                            None => bufor.clone(),}.to_rgba8()
                    } else {
                        match zaszumianie_zmienna{
                            Some(x) => zaszumianie(x,bufor.clone().resize(docelowy_wymiar, docelowy_wymiar, filtr)),
                            None => bufor.clone().resize(docelowy_wymiar, docelowy_wymiar, filtr),}.to_rgba8()
                    };
                    ...
                    (raw, "_hc16", image::ExtendedColorType::Rgba8)
                },
                ObrazTGA::TrueColor24 => {
                    ...
                    let res = if docelowy_wymiar == 0 {
                        ...
                        match zaszumianie_zmienna{
                            Some(x) => zaszumianie(x,img.resize(docelowy_wymiar, docelowy_wymiar, filtr)),
                            None => img.resize(docelowy_wymiar, docelowy_wymiar, filtr),}.to_rgb8()
                    };
                    ...
                    //     raw.push(p[0]); // Red
                    // }
                    (res.into_raw(), "_tc24", image::ExtendedColorType::Rgb8)
                },

                ObrazTGA::TrueColorA32 => {
                    println!("[debug] tga tc32");
                    ...
                    // 2. Mapujemy na BGRA: [R, G, B, A] -> B, G, R, A
                    // let mut raw = Vec::with_capacity((res.width() * res.height() * 4) as usize);
                    // for p in res.pixels() {
                    //     raw.push(p[2]); // Blue
                    //     raw.push(p[1]); // Green
                    //     raw.push(p[0]); // Red
                    //     raw.push(p[3]); // Alpha
                    // }
                    (res.into_raw(), "_tc32", image::ExtendedColorType::Rgba8)
                },
            };
            ...
            encoder.encode(&final_img, szer, wys, color_type)
