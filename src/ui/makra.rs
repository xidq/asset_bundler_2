...
macro_rules! styl_pick_list {
    ($kolor_akcentu:expr,$kolor_tla:expr) => {
                ...
                move |_theme: &iced::Theme, _status: iced::widget::pick_list::Status| {
                    ...
                    let kolor_akcentu = Color::from_rgba($kolor_akcentu.0, $kolor_akcentu.1, $kolor_akcentu.2, 0.5);
                    ...
                    pick_list::Style {
                        ...
                        placeholder_color: Color { a: 0.5, ..Color::WHITE },
                        handle_color: Color::WHITE,
                        background: tlo_bazowe.into(),
                        border: Border {
                            radius: 5.0.into(),
                            width: if matches!(_status, pick_list::Status::Hovered) { 1.0 } else { 1.0 },
                            color: if matches!(_status, pick_list::Status::Hovered) { kolor_akcentu } else { kolor_border },
                        },
                    }
                }
...
macro_rules! styl_menu_pick {
    ($kolor_zaznaczenia:expr, $kolor_tla:expr) => {
        move |_theme| {
                ...
                let kolor_zaznaczenia = Color::from_rgba($kolor_zaznaczenia.0, $kolor_zaznaczenia.1, $kolor_zaznaczenia.2, 0.5); // TWÓJ KOLOR
    ...
    ($kolor_akcentu:expr, $kolor_tla:expr) => {
        (move |_theme: &iced::Theme, _status: iced::widget::checkbox::Status| {
            ...
            checkbox::Style {
                ...
                background: if czy_zaznaczony { kolor_akcentu.into() } else { tlo_bazowe.into() },

                // Kolor "ptaszka" (widoczny tylko gdy zaznaczony)
                icon_color: Color{ a: 0.7, ..Color::WHITE },

                // Kolor tekstu obok checkboxa
                text_color: Some(Color{ a: 0.7, ..Color::WHITE }),

                border: Border {
                    radius: 3.0.into(), // Lekko mniejszy promień dla małego kwadratu
                    width: if czy_hover { 1.0 } else { 0.5 },
                    color: if czy_hover { kolor_akcentu } else { Color { a: 0.2, ..Color::WHITE } },
                },
            }
        })
    };
        ...
        (move |_theme: &iced::Theme, _status: iced::widget::text_input::Status| {
            ...
            text_input::Style {
                ...
                background: if czy_disabled {
                    Color { a: 0.5, ..tlo_bazowe }.into()
                } else {
                    tlo_bazowe.into()
                },

                value: if czy_disabled { Color { a: 0.3, ..Color::WHITE } } else { Color{ a: 0.7, ..Color::WHITE }},

                placeholder: Color { a: 0.3, ..Color::WHITE },

                border: Border {
                    radius: 5.0.into(),
                    // Świecimy ramką jeśli hover LUB focused
                    width: if (czy_hover || czy_focused) && !czy_disabled { 1.0 } else { 0.5 },
                    color: if (czy_hover || czy_focused) && !czy_disabled {
                        kolor_akcentu
                    } else {
                        Color { a: 0.1, ..Color::WHITE }
                    },
                },

                icon: Color::WHITE,
                selection: Color { a: 0.3, ..kolor_akcentu },
            }
        })
    };
}