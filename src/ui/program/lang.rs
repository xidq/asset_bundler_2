...
// pub(crate) fn odmiana_liczbowa<T>(liczba: T) -> String
// where
//     T: Into<i64> + std::fmt::Display + Copy
// {
//     let n: i64 = liczba.into();
//     let n_abs = n.abs();
//     let ostatnia_cyfra = n_abs % 10;
//     let dwie_ostatnie = n_abs % 100;
//
//     let zakonczenie = match (n_abs, ostatnia_cyfra, dwie_ostatnie) {
//         // TYLKO dla liczby 1 (jeden plik)
//         (1, _, _) => "numbers_1",
//
//         // Nastolatki (12, 13, 14) -> plików (numbers_3)
//         (_, _, 12..=14) => "numbers_3",
//
//         // Końcówki 2, 3, 4 (np. 22, 23, 24) -> pliki (numbers_2)
//         (_, 2..=4, _) => "numbers_2",
//
//         // Wszystko inne (w tym 0, 5, 10, 11, 21, 1001) -> plików (numbers_3)
//         _ => "numbers_3",
//     };
//
//     format!("{}", zakonczenie)
// }
pub(crate) fn zmieniacz_ilosci_bajtow<T>(liczba: T) -> String
where
    T: TryInto<u64> + std::fmt::Display + Copy,
    <T as TryInto<u64>>::Error: std::fmt::Debug,{
    let n: u64 = liczba.try_into().expect("Błąd: Liczba jest ujemna lub za duża dla u64");
    let n_f = n as f32;
    let bity = 8_f32;
    let kibi = 1024_u32;
...
pub(crate) fn status_to_string(status: ProcesStatus) -> &'static str{
    match status {
        ProcesStatus::Rozpoczęte => "status_note_started",
        ProcesStatus::Wtrakcie => "status_note_pending",
        ProcesStatus::IO => "status_note_io",
        ProcesStatus::Zakończone => "status_note_finished",
    }
}
    ...
    pub fn t(&self, klucz: &str) -> &'static str {
        match self {
            WybórJęzyka::PL => match klucz {
                ...
                "foto_edit_menu_extensions" => "Rozszerzenia",
                "foto_edit_menu_paths" => "Ścieżki",

                "general_off" => "wyłączone",

                "foto_edit_noising" => "Zaszumianie",

                "foto_edit_interpolation" => "Interpolacja",
                ...
                "log_status_welcome_msg_welcome" => "Dzień dobry",
                ...
                "log_status_operation_decompression" => "Dekompresja",
                ...
                "interpolacja_nearest" => "nearest",
                "interpolacja_triangle" => "triangle" ,
                "interpolacja_catmull" => "catmull",
                "interpolacja_gaussian" => "gaussian",
                "interpolacja_lanczos" => "lanczos",
            ...
            WybórJęzyka::EN => match klucz {
                ...
                "btn_proces_lack_of_data" => "Missing data",

                "interpolacja_nearest" => "Nearest",
                "interpolacja_triangle" => "Triangle" ,
                "interpolacja_catmull" => "Catmull-Rom",
                "interpolacja_gaussian" => "Gaussian",
                "interpolacja_lanczos" => "Lanczos",
            ...
            WybórJęzyka::JP => match klucz {
                ...
                "interpolacja_nearest" => "最近傍補間",
                "interpolacja_triangle" => "トライアングル" ,
                "interpolacja_catmull" => "Catmull-Rom法",
                "interpolacja_gaussian" => "ガウスぼかし",
                "interpolacja_lanczos" => "ランチョス法",
                _ => "翻訳なし",
            },
            WybórJęzyka::KR => match klucz {
                ...
                "interpolacja_nearest" => "최근접 이웃",
                "interpolacja_triangle" => "트라이앵글" ,
                "interpolacja_catmull" => "Catmull-Rom",
                "interpolacja_gaussian" => "가우시안",
                "interpolacja_lanczos" => "란초스",
                _ => "번역 없음",
            },
            WybórJęzyka::TH => match klucz {
                ...
                "interpolacja_nearest" => "จุดใกล้ที่สุด",
                "interpolacja_triangle" => "สามเหลี่ยม" ,
                "interpolacja_catmull" => "Catmull-Rom",
                "interpolacja_gaussian" => "เกาส์เซียน",
                "interpolacja_lanczos" => "Lanczos",
                _ => "ไม่มีคำแปล",
            },
        }
